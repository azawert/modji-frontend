...
const randomDate = (): dayjs.Dayjs => {
	...
	const randomTimestamp = Math.floor(Math.random() * (maxDate.unix() - currentDate.unix())) + currentDate.unix();
...
describe('formatDate', () => {
	...
	it('Кооректно отображает диапозон дат', () => {
		...
		expect(formatDate({ end, start })).toEqual(`${dayjs(start).format(DATE_FRONT_FORMAT)} - ${dayjs(end).format(DATE_FRONT_FORMAT)}`);
	});

	it('Корректно отображается только с датой окончания', () => {
		const { end } = randomDateRange()
		expect(formatDate({ end, start: null })).toEqual(` - ${dayjs(end).format(DATE_FRONT_FORMAT)}`);
	});

	it('Корректно отображается со стартовой датой', () => {
		const { start } = randomDateRange()
		expect(formatDate({ start, end: null })).toEqual(`${dayjs(start).format(DATE_FRONT_FORMAT)} - `);
	});
